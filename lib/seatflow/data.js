...
export function generateSeedTables() {
  // 12 tables split across 3 sections (indices 0-3 Section A, 4-7 Section B, 8-11 Section C)
  const cfg = [
  ...
  return cfg.map((t, i) => ({
    ...
    capacity: t.capacity,
    status: statuses[i],
    // Section slot: 0-3 → A, 4-7 → B, 8-11 → C. sectionId will be filled after loading sections.
    sectionSlot: i < 4 ? 0 : i < 8 ? 1 : 2,
    ...
    customerPhone: statuses[i] === 'occupied' ? `+91 9${randInt(100000000, 999999999)}` : null,
